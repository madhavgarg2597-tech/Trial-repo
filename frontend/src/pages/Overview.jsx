...
export default function Overview() {
  const [gestureControlEnabled, setGestureControlEnabled] = useState(systemStatus.isActive);
    ...
    <div className="min-h-screen p-8">
      ...
      >
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3 px-5 py-3 rounded-xl glass-effect border border-white/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatusCard
          ...
          subtitle={`Uptime: ${systemStatus.uptime}`}
          icon={Activity}
          status={systemStatus.isActive ? 'active' : 'error'}
        />
        <StatusCard
          title="Camera"
          value={systemStatus.cameraConnected ? 'Connected' : 'Disconnected'}
          subtitle="Live feed active"
          icon={Camera}
          status={systemStatus.cameraConnected ? 'ready' : 'error'}
        />
        <StatusCard
          title="Model Status"
          value={systemStatus.modelStatus === 'ready' ? 'Ready' : 'Training'}
          subtitle={`Confidence: ${systemStatus.confidence}%`}
          icon={Zap}
          status={systemStatus.modelStatus}
        />
        <StatusCard
